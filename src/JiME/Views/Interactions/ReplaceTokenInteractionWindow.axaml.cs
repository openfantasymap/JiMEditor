...
namespace JiME.Views
{
	...
	public partial class ReplaceTokenInteractionWindow : Window, INotifyPropertyChanged
	{
		...
		async Task<bool> TryClosing()
		{
			...
			{
				await MessageBox.Show( this, "This Event can't replace or be replaced with itself.", "Data Error", MessageBoxButton.OK, MessageBoxImage.Error );
				return false;
			}

			if ( interaction.eventToReplace == "None" || interaction.replaceWithEvent == "None" )
			{
				await MessageBox.Show( this, "The target and source Events can't be set to None.", "Data Error", MessageBoxButton.OK, MessageBoxImage.Error );
				return false;
			}

			if ( interaction.eventToReplace == interaction.replaceWithEvent )
			{
				await MessageBox.Show( this, "The target and source replacement Events can't be the same.", "Data Error", MessageBoxButton.OK, MessageBoxImage.Error );
				return false;
			}
