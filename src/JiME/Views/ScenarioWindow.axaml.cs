...
namespace JiME.Views
{
	...
	public partial class ScenarioWindow : Window
	{
		...
		async Task<bool> TryClose()
		{
			...
			{
				await MessageBox.Show( this, "The Scenario Title can't be empty.", "Invalid Scenario Title", MessageBoxButton.OK, MessageBoxImage.Error );
				return false;
			}
		...
		private async void EditIntroButton_Click( object sender, RoutedEventArgs e )
		{
		...
		private async void ResolutionEditButton_Click( object sender, RoutedEventArgs e )
		{
			...
			if ( await te.ShowDialog<bool?>( this ) == true )
			{
				...
				scenario.resolutionObserver[resolutionCB.SelectedIndex].triggerName = ( te.triggerLB.SelectedItem as Trigger )?.dataName ?? "";
		...
		private async void AddResolutionButton_Click( object sender, RoutedEventArgs e )
		{
			...
			if ( await te.ShowDialog<bool?>( this ) == true )
			{
				...
				data.triggerName = ( te.triggerLB.SelectedItem as Trigger )?.dataName ?? "";
		...
		{
			ThreatItem item = ( (ComboBox)sender ).SelectedItem as ThreatItem;
			if ( item != null )
			{
		...
		private async void specialBtn_Click( object sender, RoutedEventArgs e )
		{
